...
async def register_user(user_id: int, username: str, full_name: str, referred_by: int = None) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
...
def build_movie_text(details: dict, media_type: str) -> str:
    ...
    if media_type == "movie":
        ...
        s = details.get("number_of_seasons")
        e = details.get("number_of_episodes")
        dur = (f"📺 <b>Сезонов:</b> {s}" + (f" ({e} эп.)" if e else "") + "\n") if s else ""

    rating_str = (
        f"{stars_rating(vote_avg)} <b>{vote_avg:.1f}/10</b> ({vote_count:,} гол.)"
        if vote_count else "Нет рейтинга"
    )
...
async def cmd_start(message: Message):
    ...
    text = (
        ...
        "• Напишите название фильма или сериала — найду сразу\n"
        "• Покажу постер, описание, рейтинг и ссылку для просмотра\n"
...
async def show_referrals(message: Message):
    ...
    ref_list = await get_referral_list(user.id)
    ...
    text = (
        ...
        f"🔗 <b>Ваша ссылка:</b>\n<code>{link}</code>\n\n"
        f"📊 <b>Статистика:</b> приглашено <b>{count}</b> чел.\n"
    )
    ...
    text += "\nПоделитесь ссылкой с друзьями — они появятся здесь!"

    buttons = [
        [InlineKeyboardButton(
            text="📤 Поделиться ссылкой",
            url=f"https://t.me/share/url?url={link}&text=Ищи%20фильмы%20в%20этом%20боте!",
        )],
        [InlineKeyboardButton(text="🔄 Обновить статистику", callback_data="refresh_refs")],
    ]
...
async def handle_text(message: Message):
    ...
    for i, m in enumerate(results, 1):
        row.append(InlineKeyboardButton(
            text=str(i),
            callback_data=f"mv_{m['id']}_{m['media_type']}"
        ))
        if len(row) == 5:
            buttons.append(row)
            row = []
...
async def show_details(callback: CallbackQuery):
    ...
    watch_url = "https://www.google.com/search?q=" + urllib.parse.quote(
        f"{title} {year} смотреть онлайн бесплатно"
    )
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="referrals", description="Рефералы и статистика"),
    ])
